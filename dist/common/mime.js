...
exports.MIME_UNKNOWN = exports.MIME_BINARY = exports.MIME_TEXT = void 0;
...
exports.MIME_TEXT = MIME_TEXT;
const MIME_BINARY = 'application/octet-stream';
exports.MIME_BINARY = MIME_BINARY;
const MIME_UNKNOWN = 'application/unknown';
exports.MIME_UNKNOWN = MIME_UNKNOWN;
...
function toTextMimeAssociationItem(association) {
  return {
    ...
    filepatternOnPath: association.filepattern ? association.filepattern.indexOf(_path.posix.sep) >= 0 : false
  };
}
/**
...
function guessMimeTypes(resource, firstLine) {
  ...
  if (resource) {
    switch (resource.scheme) {
      case _network.Schemas.file:
        path = resource.fsPath;
        break;

      case _network.Schemas.data:
        const metadata = _resources.DataUri.parseMetaData(resource);

        path = metadata.get(_resources.DataUri.META_DATA_LABEL);
        break;
  ...
  const filename = (0, _path.basename)(path); // 1.) User configured mappings have highest priority
  ...
  for (let i = associations.length - 1; i >= 0; i--) {
    ...
    if (association.filepattern) {
      if (!patternMatch || association.filepattern.length > patternMatch.filepattern.length) {
        ...
        if ((0, _glob.match)(association.filepatternLowercase, target)) {
...
function guessMimeTypeByFirstline(firstLine) {
  if ((0, _strings.startsWithUTF8BOM)(firstLine)) {
...
function suggestFilename(mode, prefix) {
  const extensions = registeredAssociations.filter(assoc => !assoc.userConfigured && assoc.extension && assoc.id === mode).map(assoc => assoc.extension);
  const extensionsWithDotFirst = (0, _arrays.coalesce)(extensions).filter(assoc => assoc.startsWith('.'));
