...
const CACHE = new _map.LRUCache(10000); // bounded to 10000 elements
...
function trivia2(base, originalPattern) {
  ...
  const parsedPattern = function parsedPattern(path, basename) {
...
function parse(arg1, options = {}) {
  ...
  if (typeof arg1 === 'string' || isRelativePattern(arg1)) {
    ...
    const resultPattern = function resultPattern(path, basename) {
...
function parsedExpression(expression, options) {
  ...
  if (!parsedPatterns.some(parsedPattern => !!parsedPattern.requiresSiblings)) {
    ...
    const resultExpression = function resultExpression(path, basename) {
      for (let i = 0, n = parsedPatterns.length; i < n; i++) {
  ...
  const resultExpression = function resultExpression(path, basename, hasSibling) {
    let name = undefined;
  ...
  if (value) {
    ...
    if (typeof when === 'string') {
      const result = (path, basename, name, hasSibling) => {
        ...
        return (0, _async.isThenable)(matched) ? matched.then(m => m ? pattern : null) : matched ? pattern : null;
      };
...
function aggregateBasenameMatches(parsedPatterns, result) {
  ...
  const aggregate = function aggregate(path, basename) {
    if (typeof path !== 'string') {
      return null;
    }
    ...
    if (!basename) {
      ...
      for (i = path.length; i > 0; i--) {
        ...
        if (ch === _charCode.CharCode.Slash || ch === _charCode.CharCode.Backslash) {
          break;
        }
      }
