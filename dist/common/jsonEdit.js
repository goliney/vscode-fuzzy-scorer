...
function setProperty(text, originalPath, value, formattingOptions, getInsertionIndex) {
  ...
  const errors = [];
  const root = (0, _json.parseTree)(text, errors);
  ...
  if (!parent) {
    ...
    const existing = (0, _json.findNodeAtLocation)(parent, [lastSegment]);
...
function withFormatting(text, edit, formattingOptions) {
  ...
  if (edit.length === 0 || edit.content.length === 0) {
    // insert or remove
    while (begin > 0 && !(0, _jsonFormatter.isEOL)(newText, begin - 1)) {
      begin--;
    }

    while (end < newText.length && !(0, _jsonFormatter.isEOL)(newText, end)) {
      end++;
    }
  }

  const edits = (0, _jsonFormatter.format)(newText, {
    offset: begin,
    length: end - begin
...
function applyEdits(text, edits) {
  let sortedEdits = (0, _arrays.mergeSort)(edits, (a, b) => {
