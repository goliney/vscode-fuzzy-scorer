...
exports.GraphemeBreakType = exports.UTF8_BOM_CHARACTER = exports.UNUSUAL_LINE_TERMINATORS = void 0;
...
function firstNonWhitespaceIndex(str) {
  for (let i = 0, len = str.length; i < len; i++) {
    ...
    if (chCode !== _charCode.CharCode.Space && chCode !== _charCode.CharCode.Tab) {
      return i;
    }
  }
...
function nextCharLength(str, offset) {
  ...
  offset += initialCodePoint >= _uint.Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1;
  ...
  while (offset < len) {
    ...
    offset += nextCodePoint >= _uint.Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1;
...
function prevCharLength(str, offset) {
  ...
  while (offset > 0) {
    ...
    offset -= prevCodePoint >= _uint.Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1;
...
function _getCharContainingOffset(str, offset) {
  ...
  offset += initialCodePoint >= _uint.Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1; // extend to the right
...
function encodeUTF8(str) {
  ...
  while (strOffset < strLen) {
    ...
    strOffset += codePoint >= _uint.Constants.UNICODE_SUPPLEMENTARY_PLANE_BEGIN ? 2 : 1;
...
const UNUSUAL_LINE_TERMINATORS = /[\u2028\u2029\u0085]/; // LINE SEPARATOR (LS), PARAGRAPH SEPARATOR (PS), NEXT LINE (NEL)
...
const UTF8_BOM_CHARACTER = String.fromCharCode(_charCode.CharCode.UTF8_BOM);
exports.UTF8_BOM_CHARACTER = UTF8_BOM_CHARACTER;

function startsWithUTF8BOM(str) {
  return !!(str && str.length > 0 && str.charCodeAt(0) === _charCode.CharCode.UTF8_BOM);
}
...
function singleLetterHash(n) {
  const LETTERS_CNT = _charCode.CharCode.Z - _charCode.CharCode.A + 1;
  n = n % (2 * LETTERS_CNT);

  if (n < LETTERS_CNT) {
    return String.fromCharCode(_charCode.CharCode.a + n);
  }

  return String.fromCharCode(_charCode.CharCode.A + n - LETTERS_CNT);
...
exports.GraphemeBreakType = GraphemeBreakType;
...
class GraphemeBreakTree {
  ...
  getGraphemeBreakType(codePoint) {
    ...
    if (codePoint < 32) {
      if (codePoint === _charCode.CharCode.LineFeed) {
        return GraphemeBreakType.LF;
      }

      if (codePoint === _charCode.CharCode.CarriageReturn) {
