...
import * as scorer from '../src/index';
import { URI } from '../src/vscode_common/uri';
import { basename, dirname, sep, posix, win32 } from '../src/vscode_common/path';
import { isWindows } from '../src/vscode_common/platform';
import { Schemas } from '../src/vscode_common/network';
...
function _doScore2(target: string, query: string, matchOffset = 0): scorer.FuzzyScore2 {
  const preparedQuery = scorer.prepareQuery(query);
...
function scoreItem<T>(
  ...
  query: string,
  fuzzy: boolean,
  accessor: scorer.IItemAccessor<T>
): scorer.IItemScore {
  return scorer.scoreItemFuzzy(
    item,
...
describe('Fuzzy Scorer', () => {
  ...
  test('prepareQuery', () => {
    ...
    assert.equal(
      scorer.prepareQuery('model Tester.ts').originalLowercase,
    ...
    const restoredQuery = scorer.pieceToQuery(query.values!);
